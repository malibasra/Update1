  ...
  return (
    <div>
      <Navbar/>
        ...
        <h1 style={{ fontSize: '100px' }}>Vegetable</h1>
      <div className="container">
        <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 justify-content-center">
          {image.filter(image => image.ProductCategoryFID === 5).length > 0 ? (
            image.filter(image => image.ProductCategoryFID === 5).map((image, index) => (
